...
class DataValidator:
    ...
    def impute_missing(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        expected_range = pd.date_range(
            start=data.index.min(), end=data.index.max(), freq="h"
        )
        ...
        data["high"] = data["high"].fillna(
            pd.concat([data["open"], data["close"]], axis=1).max(axis=1)
        )
        data["low"] = data["low"].fillna(
            pd.concat([data["open"], data["close"]], axis=1).min(axis=1)
        )
    ...
    def validate_and_impute(
        self, symbol: str, start_date: str, end_date: str, expected_freq="h"
    ) -> pd.DataFrame:
        """
