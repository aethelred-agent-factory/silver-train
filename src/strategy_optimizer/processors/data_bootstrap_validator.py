...
class DataBootstrapValidator:
    ...
    def __init__(self, config):
        ...
        self.required_symbols = config["system_config"]["data_sources"][
            "follower_assets"
        ] + [config["system_config"]["data_sources"]["leader_asset"]]
        logging.info(
            f"DataBootstrapValidator initialized. Required symbols: {self.required_symbols}"
        )

    def validate_symbol_data(
        self, symbol: str, min_candles: int = 100
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if num_candles < min_candles:
                return (
                    False,
                    f"Insufficient candles for {symbol}: {num_candles} < {min_candles}",
                )
            ...
            if (df["volume"] == 0).all():
                return (
                    False,
                    f"All volumes are zero for {symbol} - data may be corrupted",
                )

            return (
                True,
                f"✓ {symbol}: {num_candles} candles, range {df['timestamp'].min()} to {df['timestamp'].max()}",
            )

        except Exception as e:
            return False, f"Error reading {symbol} data: {str(e)}"

    def validate_all_required_symbols(
        self, min_candles: int = 100
    ) -> Tuple[bool, List[str]]:
        """
