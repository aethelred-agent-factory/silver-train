...
class IndicatorEngine:
    ...
    def compute_leader_correlation(
        self, data: pd.DataFrame, leader_data: pd.DataFrame, window: int = 30
    ) -> pd.Series:
        """
    ...
    def compute_atr_volatility_percentile(
    ...
    def compute_all(
        self, symbol: str, start_date: str, end_date: str, leader_symbol: str = None
    ) -> pd.DataFrame:
        """
        ...
        """
        logging.info(
            f"Computing all indicators for {symbol} from {start_date} to {end_date}"
        )
        ...
        indicators["volatility_percentile"] = self.compute_atr_volatility_percentile(
            data
        )
        ...
        if leader_symbol:
            try:
                leader_data = self.market_data_bus.get_candles(
                    leader_symbol, start_date, end_date
                )
                if not leader_data.empty:
                    indicators["leader_correlation"] = self.compute_leader_correlation(
                        data, leader_data
                    )
                else:
                    logging.warning(
                        f"No data found for leader {leader_symbol}, skipping correlation."
                    )
