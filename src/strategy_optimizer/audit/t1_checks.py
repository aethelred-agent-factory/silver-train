...
class T1Checks:
    ...
    def check_contradiction(self, proposal: OptimizerProposal) -> TieredFinding:
        ...
        if (
            context.get("regime_label") == "RANGE"
            and metrics.get("atr_percentile_90", 0)
            > self.config["regime_config"]["atr_percentile_thresholds"][
                "high_volatility"
            ]
        ):
            rule = next(
                (r for r in self.audit_rules if r["name"] == "T1_Contradiction"), None
            )
            if rule:
                return TieredFinding(
                    tier="T1", code=rule["name"], explanation=rule["description"]
                )
        return None
            ...
            if param in self.parameter_bounds:
                ...
                if not (bounds["min"] <= value <= bounds["max"]):
                    rule = next(
                        (
                            r
                            for r in self.audit_rules
                            if r["name"] == "T1_BoundsViolation"
                        ),
                        None,
                    )
                    if rule:
    ...
    def check_data_integrity(self, proposal: OptimizerProposal) -> TieredFinding:
        ...
        """
        data_gap_pct = proposal.context.get("metrics_snapshot", {}).get(
            "missing_data_pct", 0
        )
        gap_threshold = self.config["system_config"]["thresholds"].get(
            "t1_data_gap_threshold", 0.1
        )

        if data_gap_pct > gap_threshold:
            rule = next(
                (r for r in self.audit_rules if r["name"] == "T1_DataGap"), None
            )
            if rule:
                return TieredFinding(
                    tier="T1",
    ...
    def check_circularity(self, proposal: OptimizerProposal) -> TieredFinding:
        """
        Legacy circularity heuristic used in tests: proposals with a very high
        proposal_version are considered suspiciously circular.
        """
        # Simple heuristic for tests: versions > 100 flagged as circular
        if getattr(proposal, "proposal_version", 0) > 100:
            rule = next(
                (r for r in self.audit_rules if r["name"] == "T1_Circularity"), None
            )
            if rule:
                return TieredFinding(
                    tier="T1", code=rule["name"], explanation=rule["description"]
