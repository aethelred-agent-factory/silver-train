...
class PerformanceMetrics:
    ...
    def calculate_sharpe_ratio(
        self, equity_curve: pd.Series, periods_per_year=252 * 24
    ) -> float:
        ...
        if returns.std() == 0:
            return 0
        return (
            (returns.mean() - self.risk_free_rate / periods_per_year)
            / returns.std()
    ...
    def calculate_all_metrics(
        self, equity_curve: pd.Series, total_trades: int, winning_trades: int
    ) -> dict:
