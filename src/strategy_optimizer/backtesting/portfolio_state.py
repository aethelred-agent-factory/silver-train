...
class PortfolioState:
    ...
    def enter_position(
        ...
        if quantity <= 0:
            logging.warning(
                f"Cannot enter position with non-positive quantity: {quantity}"
            )
            return False
        ...
        logging.info(
            f"Entered {symbol}: {quantity} units at {entry_price}, cash remaining: {self.cash}"
        )
        return True

    def exit_position(
        self, symbol: str, exit_price: float, timestamp: Optional[datetime] = None
    ) -> Optional[Dict]:
        """
        ...
        logging.info(
            f"Exited {symbol}: {quantity} units at {exit_price}, PnL: {pnl:.2f} ({pnl_pct:.2f}%)"
        )
        return trade
    ...
    def get_metrics(self) -> Dict:
        ...
        avg_win = (
            sum(t["pnl"] for t in self.trade_history if t["is_winner"]) / winning_trades
            if winning_trades > 0
            else 0
        )
        avg_loss = (
            abs(
                sum(t["pnl"] for t in self.trade_history if not t["is_winner"])
                / losing_trades
            )
            if losing_trades > 0
            else 0
        ...
        profit_factor = (
            avg_win / avg_loss if avg_loss > 0 else (2.0 if avg_win > 0 else 1.0)
        )
