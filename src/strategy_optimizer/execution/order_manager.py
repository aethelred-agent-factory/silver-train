...
class OrderManager:
    ...
    def place_order(
        self, symbol: str, side: str, amount: float, proposal: Any, price: float = None
    ) -> str:
        """
        Places an order on the exchange and logs it.
        """
        logging.info(
            f"Placing {side} order for {amount} {symbol} at price {price if price else 'market'}"
        )
        ...
        try:
            ...
            order_info = self.exchange_adapter.create_order(
    ...
    def track_fill(self, order_id: str):
        ...
        try:
            ...
            logging.info(
                f"Order {order_id} status updated to {status}. Filled: {filled_amount} @ {filled_price}"
            )
        except Exception as e:
