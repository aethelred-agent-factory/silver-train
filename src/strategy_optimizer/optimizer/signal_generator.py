...
class SignalGenerator:
    ...
    def calculate_signal_score(
        self, indicators: pd.DataFrame, params: dict
    ) -> pd.DataFrame:
        """
        ...
        rsi_score = (
            (rsi_overbought - rsi_clamped) / (rsi_overbought - rsi_oversold)
        ).clip(0, 1)
        ...
        if volume_sma.isna().sum() > len(volume_sma) * 0.5:
            ...
            volume_score = (
                (indicators["volume"] > volume_median).astype(float).fillna(0.7)
            )  # 0.7 instead of 0.5
        else:
    ...
    def generate_signals(
        self, symbol: str, start_date: str, end_date: str, params: dict
    ) -> pd.DataFrame:
        """
