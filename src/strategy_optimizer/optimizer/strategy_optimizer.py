...
class StrategyOptimizer:
    ...
    def propose_parameters(
        ...
        """
        logging.info(
            f"Iteration {iteration}: Generating parameter proposal for regime: {regime}"
        )
        ...
        if self.stability_guards:
            ...
            prev_drawdown = (
                self.previous_metrics.get("max_drawdown_pct", 0)
            ...
            is_frozen = self.stability_guards.check_freeze(
                iteration, current_metrics.get("profit", 0), current_drawdown
            )

            if is_frozen:
                logging.info("Parameters are frozen, returning HOLD")
                return (
                    self.config.get("safe_baseline", {}),
                    "HOLD",
                    "Parameters frozen for stability",
                )

            # Check rollback
            rollback_params = self.stability_guards.check_rollback(
                iteration, current_drawdown, prev_drawdown
            )
            ...
            if rollback_params:
                ...
                return (
                    rollback_params,
                    "ROLLBACK",
                    "Rolled back due to drawdown increase > 15%",
                )

        # 1. Build optimization context for LLM
        current_params = current_metrics.get(
            "parameters", self.config.get("safe_baseline", {})
        )

        # Check if parameter set has been tested before
        tested_before = (
            self.parameter_memory.has_been_tested(current_params, regime)
        ...
        optimization_context = {
            ...
            "stability_state": self.stability_guards.get_stability_state()
            if self.stability_guards
            else "NORMAL",
            ...
            "best_parameters": self.parameter_memory.get_best_params(regime)
            if self.parameter_memory
            else {},
            "improved_regimes": improved_regimes,
            "drawdown_decreasing": (
                self.previous_metrics
                and current_metrics.get("max_drawdown_pct", 0)
                < self.previous_metrics.get("max_drawdown_pct", 0)
            )
            if self.previous_metrics
            else False,
...
Suggest parameter adjustments or hold current parameters."""
        ...
        llm_response = (
            self.llm_interface.query_llm(prompt, optimization_context)
        ...
        if not llm_response:
            ...
            proposed_params = (
                self.fallback_mode.perturb_parameters(
                    current_params, "BTC/USDT", end_date
                )
        ...
        if self.parameter_memory:
            self.parameter_memory.log_tested(
                proposed_params if action == "UPDATE" else current_params,
                regime,
                current_metrics,
                action,
            )
        ...
        if self.stability_guards and current_metrics.get("profit", 0) > 0:
            self.stability_guards.record_stable_state(
                iteration, current_params, current_metrics
            )
        ...
        logging.info(
            f"Proposal generated: action={action}, version={self.proposal_version}"
        )
