...
class StatisticalTests:
    ...
    def bootstrap_ci(
        self, data: pd.Series, metric_func, iterations=2000, confidence_level=0.95
    ) -> tuple:
        """
        ...
        logging.info(
            f"Bootstrap CI ({confidence_level*100}%): [{lower_bound:.4f}, {upper_bound:.4f}]"
        )
        return (lower_bound, upper_bound)

    def permutation_test(
        self, data_a: pd.Series, data_b: pd.Series, metric_func, permutations=1000
    ) -> float:
        """
    ...
    def t_test(self, data_a: pd.Series, data_b: pd.Series, equal_var=False) -> tuple:
        ...
        logging.info(
            f"Independent t-test: t-statistic={t_stat:.4f}, p-value={p_value:.4f}"
        )
