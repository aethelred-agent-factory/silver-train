...
def load_historical_data(symbol: str, start_date: str, end_date: str, timeframe: str = '1h'):
    """
    Loads historical market data for a given symbol and date range.
    """
    setup_logging()
    load_dotenv()
    print(f"Loading historical data for {symbol} from {start_date} to {end_date} ({timeframe})...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start", type=str, required=True, help="Start date (ISO 8601 format, e.g., 2023-01-01T00:00:00Z)")
    parser.add_argument("--end", type=str, required=True, help="End date (ISO 8601 format, e.g., 2023-01-31T23:59:59Z)")
    parser.add_argument("--timeframe", type=str, default="1h", help="Timeframe (e.g., 1m, 5m, 1h, 1d)")
