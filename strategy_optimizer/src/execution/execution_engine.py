...
class ExecutionEngine:
    ...
    """
    def __init__(self, config, order_manager: OrderManager, state_manager, portfolio_state=None):
        self.config = config
    ...
    def execute_trade(self, signal: dict, proposal: OptimizerProposal, audit_verdict: AuditVerdict) -> bool:
        """
        ...
        logging.info(f"Attempting to execute trade for {signal.get('symbol')} | Proposal: {proposal.proposal_id}")
        ...
        try:
            order_id = self.order_manager.place_order(symbol, side, amount, proposal, price=price)
    ...
    def _check_capital_exposure(self, signal: dict) -> bool:
        ...
        max_exposure = self.config.get('system_config', {}).get('risk', {}).get('max_capital_exposure_pct', 20.0)
        ...
        if exposure_pct > max_exposure:
            logging.warning(f"Capital Exposure ({exposure_pct:.2f}%) exceeds limit ({max_exposure}%)")
            return False
        return True
    ...
    def _check_risk_constraints(self) -> bool:
        ...
        max_drawdown = self.config.get('system_config', {}).get('risk', {}).get('max_drawdown_limit_pct', 15.0)
        current_drawdown = self.portfolio_state.max_drawdown_pct
        
        if current_drawdown > max_drawdown:
            logging.warning(f"Portfolio Drawdown ({current_drawdown:.2f}%) exceeds limit ({max_drawdown}%)")
            return False
        return True
