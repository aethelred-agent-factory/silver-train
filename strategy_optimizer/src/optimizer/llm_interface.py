...
class LLMInterface:
    ...
    def query_llm(self, prompt: str, context: dict) -> dict:
        ...
        try:
            ...
            return self.parse_optimization_response(response.choices[0].message.content, context)
    ...
    def parse_optimization_response(self, response_content: str, context: dict) -> dict:
        ...
        try:
            ...
            if len(proposal.get('params', {})) > 2:
                logging.warning(f"Proposal modifies {len(proposal['params'])} params, truncating to 2")
            ...
            if context.get('global_profit', 0) > 0 and context.get('drawdown_decreasing', False):
                logging.info("Overriding UPDATE to HOLD: profit positive and drawdown decreasing")
