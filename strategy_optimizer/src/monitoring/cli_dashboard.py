...
class CLIDashboard:
    ...
    def display_iteration_summary(
        ...
        # Global Metrics
        metrics_table = self._build_metrics_table(regime, regime_confidence, global_metrics)
        layout["metrics"].update(Panel(metrics_table, title="[bold]Global Metrics[/bold]"))
        
        # Per-Regime Metrics
        regime_table = self._build_regime_table(per_regime_metrics)
        layout["regimes"].update(Panel(regime_table, title="[bold]Per-Regime Performance[/bold]"))
        
        # Stability Info
        stability_text = self._build_stability_text(stability_state, freeze_countdown)
        layout["stability"].update(Panel(stability_text, title="[bold]Stability State[/bold]", style="bold yellow" if stability_state != "NORMAL" else ""))
        
        # AI Decision
        if ai_reasoning:
            layout["ai_decision"].update(Panel(ai_reasoning, title="[bold]AI Reasoning[/bold]", style="dim"))
        
        self.console.print(layout)
    
    def _build_metrics_table(self, regime: str, confidence: float, metrics: Dict) -> Table:
        ...
        table.add_row(f"[{regime_style}]Regime[/{regime_style}]", f"[{regime_style}]{regime}[/{regime_style}]")
        table.add_row("Confidence", f"{confidence:.2%}")
        table.add_row("Profit", f"[green]{metrics.get('profit', 0):.2f}%[/green]" if metrics.get('profit', 0) > 0 else f"[red]{metrics.get('profit', 0):.2f}%[/red]")
        table.add_row("Max Drawdown", f"[red]{metrics.get('max_drawdown_pct', 0):.2f}%[/red]")
    ...
    def _build_stability_text(self, stability_state: str, freeze_countdown: Optional[int]) -> str:
        """Build the stability state text."""
        if stability_state == "FROZEN":
            countdown_text = f" - Frozen for {freeze_countdown} more iterations" if freeze_countdown else ""
    ...
    def display_parameter_update(self, old_params: Dict, new_params: Dict, action: str):
        ...
        """
        table = Table(show_header=True, header_style="bold magenta", title=f"Parameter Update: {action}")
        ...
        for param in sorted(all_params):
            ...
            if old_val != new_val:
                ...
                table.add_row(param, str(old_val), f"[green]{change_indicator} {new_val}[/green]")
            else:
    ...
    def display_error(self, error_message: str, title: str = "Error"):
        ...
        """
        self.console.print(Panel(f"[bold red]{error_message}[/bold red]", title=f"[bold red]{title}[/bold red]", style="bold red"))
    
    def display_warning(self, warning_message: str, title: str = "Warning"):
        """
        Display a warning message.
        """
        self.console.print(Panel(f"[bold yellow]{warning_message}[/bold yellow]", title=f"[bold yellow]{title}[/bold yellow]", style="bold yellow"))
    
    def display_info(self, info_message: str, title: str = "Info"):
        """
        Display an info message.
        """
        self.console.print(Panel(info_message, title=f"[bold cyan]{title}[/bold cyan]", style="bold cyan"))
