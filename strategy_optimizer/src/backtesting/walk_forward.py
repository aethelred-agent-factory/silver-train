...
class WalkForward:
    ...
    def split_datasets(self, data: pd.DataFrame, cal_pct=60, val_pct=20, test_pct=20):
        ...
        logging.info(f"Split dataset into: Calibration ({len(calibration_set)}), "
                     f"Validation ({len(validation_set)}), Test ({len(test_set)})")

        return calibration_set, validation_set, test_set

    def rolling_walk_forward(self, symbol: str, start_date: str, end_date: str, params: dict, window_size: int, step_size: int):
        """
        ...
        for i in range(0, n - window_size, step_size):
            ...
            logging.info(f"Running walk-forward iteration on test window: "
