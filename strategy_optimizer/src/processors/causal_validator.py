...
class CausalValidator:
    ...
    def validate_timestamp_causality(self, computation_timestamp: datetime, data: pd.DataFrame) -> bool:
        """
    ...
    def validate_causal_chain(self, chain: list, proposal_timestamp: datetime):
        ...
        for link in chain:
            if hasattr(link, 'timestamp') and link.timestamp > proposal_timestamp:
                logging.error(f"Causal chain violation: item in chain is from the future.")
                return False
        return True
