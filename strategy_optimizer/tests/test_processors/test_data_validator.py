# tests/test_processors/test_data_validator.py
import pytest
import pandas as pd
...
def test_impute_missing(data_validator, sample_data_with_gaps):
    ...
    assert imputed_df.loc[imputed_df['timestamp'] == datetime(2023, 1, 1, 2), 'close'].iloc[0] == 101
    assert imputed_df.loc[imputed_df['timestamp'] == datetime(2023, 1, 1, 5), 'close'].iloc[0] == 104
    assert imputed_df.loc[imputed_df['timestamp'] == datetime(2023, 1, 1, 5), 'volume'].iloc[0] == 0
