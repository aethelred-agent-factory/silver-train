...
import pandas as pd
import numpy as np
from src.utils.statistical_tests import StatisticalTests
...
def test_bootstrap_ci(statistical_tests, sample_data_series):
    lower, upper = statistical_tests.bootstrap_ci(sample_data_series, mean_func, iterations=100) # Reduced iterations for faster test
...
def test_permutation_test(statistical_tests, sample_data_series, sample_data_series_b):
    p_value = statistical_tests.permutation_test(sample_data_series, sample_data_series_b, mean_func, permutations=100) # Reduced for faster test
