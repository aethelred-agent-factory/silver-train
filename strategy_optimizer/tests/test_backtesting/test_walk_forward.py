...
def mock_backtest_engine(mocker, sample_full_data):
    ...
    mock.signal_generator.indicator_engine.market_data_bus.get_candles.return_value = sample_full_data
    return mock
...
def test_split_datasets(walk_forward, sample_full_data):
    cal, val, test = walk_forward.split_datasets(sample_full_data, cal_pct=60, val_pct=20, test_pct=20)
...
def test_rolling_walk_forward(walk_forward, mock_backtest_engine):
    ...
    results = walk_forward.rolling_walk_forward(symbol, start, end, params, window_size, step_size)
