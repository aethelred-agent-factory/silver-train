...
def test_request_approval(approval_workflow, in_memory_state_manager):
    ...
    # Verify in DB
    ticket = in_memory_state_manager.execute_query("SELECT * FROM approval_tickets WHERE ticket_id = ?", (ticket_id,), fetch='one')
    assert ticket is not None
    assert ticket[1] == incident_id
    assert ticket[3].startswith(datetime.utcnow().isoformat()[:10]) # Date part should match
...
def test_check_approval_status_expired(approval_workflow, in_memory_state_manager):
    ...
    ticket_id = approval_workflow.request_approval("inc_pqr", "Test expired", timeout_hours=-1) 
    ...
    ticket = in_memory_state_manager.execute_query("SELECT status FROM approval_tickets WHERE ticket_id = ?", (ticket_id,), fetch='one')
