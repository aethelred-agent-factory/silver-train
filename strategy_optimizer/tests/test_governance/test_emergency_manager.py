...
def test_resume_system(emergency_manager, mock_incident_tracker, mock_approval_workflow):
    emergency_manager.trigger_emergency("Test_Emergency", "test")
    emergency_manager.resume_system("incident_id_123", "Manual review and fix")
    
    assert emergency_manager.is_system_halted() is False
    mock_incident_tracker.mark_resolved.assert_called_once_with("incident_id_123", "Manual review and fix")
