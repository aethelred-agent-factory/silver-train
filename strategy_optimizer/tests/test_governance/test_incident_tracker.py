...
def test_log_incident(incident_tracker, in_memory_state_manager):
    incident_id = incident_tracker.log_incident("T1_DataGap", "Missing critical market data", "prop_123")
    
    assert incident_id is not None
    
    # Verify in DB
    incident = in_memory_state_manager.execute_query("SELECT * FROM incidents WHERE incident_id = ?", (incident_id,), fetch='one')
...
def test_mark_resolved(incident_tracker, in_memory_state_manager):
    incident_id = incident_tracker.log_incident("T1_DataGap", "Missing critical market data")
...
def test_get_incident_history(incident_tracker, in_memory_state_manager):
    ...
    filtered_history = incident_tracker.get_incident_history(incident_id=first_incident_id)
