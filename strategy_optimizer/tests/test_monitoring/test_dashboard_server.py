...
@pytest.fixture(scope="function")
def dashboard_app(test_config, mock_state_manager, mock_artifact_manager, mock_incident_tracker):
    ...
    server = DashboardServer(test_config, mock_state_manager, mock_artifact_manager, mock_incident_tracker)
    return server.app
...
async def test_get_metrics(dashboard_app):
    with TestClient(dashboard_app) as client:
        ...
        assert response.status_code == 200
        assert response.json() == {"status": "ok", "equity": 100000, "drawdown": 5.2, "t1_flags": 0}
...
async def test_get_audit_artifact(dashboard_app, mock_artifact_manager):
    with TestClient(dashboard_app) as client:
        ...
        mock_artifact_manager.download_artifact.assert_called_once_with("audit_test_audit_id.json")
