...
class EmergencyManager:
    ...
    def trigger_emergency(self, incident_type: str, details: str, proposal_id: str = None):
        """
        Triggers an emergency protocol: halts the system, logs an incident, and alerts.
        """
        if self.system_halted:
            logging.warning("System already halted. Skipping duplicate emergency trigger.")
            return
    ...
    def resume_system(self, incident_id: str, resolution: str):
        ...
        logging.info(f"System resume initiated for incident {incident_id}. Resolution: {resolution}")
        self.system_halted = False
