...
class TimeUtils:
    ...
    def to_iso8601(self, dt_object: datetime) -> str:
        ...
        if dt_object.tzinfo is None:
            ...
            return dt_object.replace(tzinfo=timezone.utc).isoformat(timespec='milliseconds').replace('+00:00', 'Z')
    ...
    def ensure_causal(self, timestamp: datetime, reference_timestamp: datetime) -> bool:
        ...
        if timestamp > reference_timestamp:
            logging.error(f"Causality violation: Timestamp ({timestamp}) is after reference ({reference_timestamp}).")
            return False
        return True
