...
class ArtifactManager:
    ...
    def __init__(self, config, crypto_utils):
        ...
        if self.use_s3:
            try:
                ...
                logging.info(f"Initialized ArtifactManager with S3 bucket: {self.s3_bucket}")
            except Exception as e:
                logging.error(f"Failed to initialize S3 client: {e}. Falling back to local storage.")
                self.use_s3 = False
        else:
            logging.info(f"Initialized ArtifactManager with local storage: {self.local_artifact_path}")
    ...
    def upload_artifact(self, data: str, artifact_id: str) -> str:
        ...
        if self.use_s3:
            try:
                ...
                logging.error(f"Failed to upload {artifact_id} to S3: {e}. Saving locally as fallback.")
    ...
    def download_artifact(self, artifact_id: str) -> tuple[str, str]:
        ...
        try:
            if self.use_s3:
                response = self.s3_client.get_object(Bucket=self.s3_bucket, Key=artifact_id)
        ...
        if data_bytes and stored_checksum:
            ...
            if computed != stored_checksum:
                logging.error(f"Checksum mismatch for artifact {artifact_id}: expected {stored_checksum}, got {computed}")
                return None, None
