...
class StabilityGuards:
    ...
    def __init__(self, config, state_manager=None, parameter_memory=None):
        ...
        self.freeze_consecutive_iterations = stability_cfg.get('consecutive_improving_for_freeze', 3)
        self.freeze_duration = stability_cfg.get('freeze_duration', 5)
        self.rollback_drawdown_threshold = stability_cfg.get('rollback_drawdown_spike', 15.0)
        self.blacklist_drawdown_threshold = stability_cfg.get('blacklist_drawdown_threshold', 40.0)
        
        # State tracking
        self.freeze_state = None  # None, or {'until_iteration': N, 'frozen_params': {...}}
        ...
        logging.info(f"  Consecutive improving for freeze: {self.freeze_consecutive_iterations}")
        logging.info(f"  Rollback drawdown spike: {self.rollback_drawdown_threshold}%")
        logging.info(f"  Blacklist drawdown threshold: {self.blacklist_drawdown_threshold}%")
    ...
    def check_freeze(self, iteration: int, current_profit: float, current_drawdown: float) -> bool:
        """
        ...
        if self.freeze_state is not None:
            if iteration >= self.freeze_state['until_iteration']:
                ...
                self._record_stability_change('NORMAL', iteration, "Freeze period expired")
            else:
                freeze_countdown = self.freeze_state['until_iteration'] - iteration
                logging.info(f"Parameters frozen: {freeze_countdown} iterations remaining")
                return True
        ...
        if current_profit > self.freeze_profit_threshold:
            ...
            if len(self.performance_history) > self.freeze_consecutive_iterations + 1:
                self.performance_history = self.performance_history[-(self.freeze_consecutive_iterations + 1):]
            ...
            if len(self.performance_history) >= self.freeze_consecutive_iterations:
                ...
                is_decreasing = all(drawdowns[i] >= drawdowns[i+1] for i in range(len(drawdowns)-1))
                
                if is_decreasing and len(set(drawdowns)) > 1:  # Avoid trivial case of all same values
                    logging.warning(f"FREEZE ACTIVATED at iteration {iteration}: "
                    ...
                    self._record_stability_change('FROZEN', iteration, 
    ...
    def check_rollback(self, iteration: int, current_drawdown: float, previous_drawdown: float) -> Optional[Dict]:
        """
        ...
        if drawdown_increase > self.rollback_drawdown_threshold and self.last_stable_params:
            logging.error(f"ROLLBACK TRIGGERED at iteration {iteration}: "
                         f"drawdown increased by {drawdown_increase:.2f}% "
                         f"(threshold: {self.rollback_drawdown_threshold}%)")
            
            self.stability_state = 'RECOVERY'
            self._record_stability_change('RECOVERY', iteration, 
                                        f"Drawdown spike: +{drawdown_increase:.2f}%")
    ...
    def record_stable_state(self, iteration: int, params: Dict, metrics: Dict):
        ...
        logging.info(f"Recorded stable state at iteration {iteration}: "
        ...
        if self.stability_state == 'RECOVERY' and profit > 0 and drawdown < 20:
            self.stability_state = 'NORMAL'
            self._record_stability_change('NORMAL', iteration, "Recovered to good performance")
    ...
    def check_blacklist(self, params: Dict, drawdown: float) -> bool:
        ...
        if drawdown > self.blacklist_drawdown_threshold:
            params_hash = self._hash_params(params)
            logging.warning(f"BLACKLISTING parameter set {params_hash}: "
                           f"drawdown {drawdown:.2f}% > {self.blacklist_drawdown_threshold}%")
            
            if self.parameter_memory:
                self.parameter_memory.blacklist(params_hash, 
    ...
    def get_freeze_countdown(self, current_iteration: int) -> int:
        ...
        if self.freeze_state is not None:
            countdown = max(0, self.freeze_state.get('until_iteration', 0) - current_iteration)
            return countdown
        return 0
    ...
    def _record_stability_change(self, new_state: str, iteration: int, reason: str = "") -> None:
    ...
    def apply_heuristic_update(self, current_params: Dict, regime: str) -> Dict:
        ...
        if regime == 'RANGE':
            ...
            adjusted['stop_multiplier'] = min(adjusted.get('stop_multiplier', 2.0) + 0.2, 3.0)
