...
class PositionManager:
    ...
    def calculate_position_size(self, capital: float, risk_pct: float, entry_price: float, stop_loss_price: float) -> float:
        """
        Calculates the position size based on the risk percentage of the capital.
        """
        if entry_price <= stop_loss_price:
            logging.warning("Stop loss price must be below entry price for a long position.")
            return 0
    ...
    def place_stop_loss(self, entry_price: float, atr: float, multiplier: float) -> float:
        """
    ...
    def place_take_profit(self, entry_price: float, atr: float, multiplier: float) -> float:
        """
