...
def run_validation(config, data, windows, params):
    ...
    for idx, (start_dt, end_dt) in enumerate(windows, 1):
        ...
        try:
            ...
            # Extract metrics
            profit_pct = ((result.profit_factor - 1) * 100) if result.profit_factor > 0 else 0
            ...
            window_result = {
                ...
                'max_drawdown_pct': result.max_drawdown_pct if result.max_drawdown_pct else 0,
            ...
            print(f"  Window {idx:2d}: {window_result['start_date']} → {window_result['end_date']:19s} | "
...
def print_summary(summary):
    ...
    print(f"  Profitable windows: {summary_stats['profitable_windows']}/{summary_stats['total_windows']} "
    ...
    print(f"  Profit range: {summary_stats['min_profit']:+.2f}% to {summary_stats['max_profit']:+.2f}%")
...
def main():
    ...
    print(f"Available data: {min_date.strftime('%Y-%m-%d %H:%M')} to {max_date.strftime('%Y-%m-%d %H:%M')}")
    print(f"Total data points: {len(data)} candles ({len(data) / 24:.1f} days at 1h bars)")
