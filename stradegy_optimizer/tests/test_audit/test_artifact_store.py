...
def mock_artifact_manager(mocker):
    ...
    mock.download_artifact.return_value = None # Default for testing retrieve non-existent
    return mock
...
def sample_audit_verdict():
    return AuditVerdict(
        ...
        action=AuditAction(type="ALLOW_WITH_RESTRICTION", restrictions={"max_order_size_pct": 0.5}),
        artifact_refs=[],
        checksum=""
    )

def test_store_artifact(artifact_store, mock_artifact_manager, sample_audit_verdict):
    mock_artifact_manager.upload_artifact.return_value = ("file:///mock/path/artifact.json", "dummy_checksum")
...
def test_retrieve_artifact_not_found(artifact_store, mock_artifact_manager):
    ...
    mock_artifact_manager.download_artifact.assert_called_once_with("audit_non_existent_audit.json")

def test_retrieve_artifact_found(artifact_store, mock_artifact_manager, sample_audit_verdict, crypto_utils):
