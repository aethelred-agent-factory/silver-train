...
def mock_signal_generator(mocker, sample_ohlcv_data):
    ...
    mock_signals['signal'] = [1 if i % 4 < 2 else 0 for i in range(len(sample_ohlcv_data))] # Buy for 2 hours, sell for 2 hours
...
@pytest.fixture
def backtest_engine(test_config, mock_signal_generator, mock_position_manager, performance_metrics):
    return BacktestEngine(test_config, mock_signal_generator, mock_position_manager, performance_metrics)
