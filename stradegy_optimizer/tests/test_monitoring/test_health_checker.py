...
@pytest.fixture
def health_checker(test_config, mock_state_manager, mock_exchange_adapter, mock_alerting):
    return HealthChecker(test_config, mock_state_manager, mock_exchange_adapter, mock_alerting)
...
def test_check_database_connection_failure(health_checker, mock_state_manager, mock_alerting):
...
def test_run_checks_all_healthy(health_checker, mock_state_manager, mock_exchange_adapter, mock_alerting):
...
def test_run_checks_db_failure(health_checker, mock_state_manager, mock_alerting):
    ...
    mock_alerting.send_urgent_alert.assert_called_once_with("Database connection failed!")
