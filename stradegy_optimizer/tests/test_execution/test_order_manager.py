...
def mock_exchange_adapter(mocker):
    ...
    def create_order_side_effect(*args, **kwargs):
        ...
        return {"id": f"mock_order_id_{call_counter['count']}", "status": "open", "price": 100.0}
    
    mock.create_order.side_effect = create_order_side_effect
    mock.fetch_order_status.return_value = {"id": "mock_order_id_1", "status": "closed", "filled": 0.01, "average": 100.1}
    return mock
...
def test_place_order(order_manager, mock_exchange_adapter, in_memory_state_manager, sample_proposal_order):
    symbol = "BTC/USDT"
    side = "buy"
    amount = 0.01
    price = 100.0
    
    order_id = order_manager.place_order(symbol, side, amount, sample_proposal_order, price)
    
    assert order_id == "mock_order_id_1"
    mock_exchange_adapter.create_order.assert_called_once_with(symbol, 'limit', side, amount, price)
    
    # Verify entry in database
    orders = in_memory_state_manager.execute_query("SELECT * FROM order_log WHERE order_id = ?", ("mock_order_id_1",), fetch='one')
    assert orders is not None
    assert orders[1] == symbol # symbol
    assert orders[5] == "open" # status

def test_track_fill(order_manager, mock_exchange_adapter, in_memory_state_manager, sample_proposal_order):
    ...
    orders = in_memory_state_manager.execute_query("SELECT * FROM order_log WHERE order_id = ?", (order_id,), fetch='one')
    assert orders[5] == "closed" # status should be updated
    assert orders[4] == 100.1 # price should be updated (average fill price)

def test_get_order_history(order_manager, in_memory_state_manager, sample_proposal_order):
