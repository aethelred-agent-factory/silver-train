...
from src.data_bus.schemas import AuditVerdict, AuditAction
...
def test_apply_restrictions_override(restriction_enforcer):
    audit_verdict1 = AuditVerdict(
        action=AuditAction(
            type="ALLOW_WITH_RESTRICTION",
            restrictions={"max_order_size_pct": 0.5}
        )
    )
    ...
    assert restriction_enforcer.get_active_restrictions() == {"max_order_size_pct": 0.25, "new_restriction": True}
...
def test_lift_restrictions(restriction_enforcer):
    ...
    assert restriction_enforcer.get_active_restrictions() == {"max_order_size_pct": 0.5, "max_daily_trades": 10}
