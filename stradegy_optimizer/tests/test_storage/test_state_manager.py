...
def test_execute_query_insert_and_fetch(state_manager_file):
    state_manager_file.execute_query("CREATE TABLE IF NOT EXISTS test_table (id INTEGER PRIMARY KEY, name TEXT);")
    state_manager_file.execute_query("INSERT INTO test_table (name) VALUES (?);", ("test_name_1",))
    state_manager_file.execute_query("INSERT INTO test_table (name) VALUES (?);", ("test_name_2",))
    
    result_one = state_manager_file.execute_query("SELECT name FROM test_table WHERE id = 1;", fetch='one')
    assert result_one == ("test_name_1",)
    
    result_all = state_manager_file.execute_query("SELECT name FROM test_table;", fetch='all')
