...
def load_config():
    ...
    # Load safe baseline for defaults
    with open(os.path.join(config_dir, 'safe_baseline.yaml')) as f:
        config['safe_baseline'] = yaml.safe_load(f)
    with open(os.path.join(config_dir, 'system_config.yaml')) as f:
        config['system_config'] = yaml.safe_load(f)
...
def demo_dashboard():
    ...
    config = load_config()
    dashboard = BlessedDashboard(config)
