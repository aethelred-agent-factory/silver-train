...
from src.data_bus.schemas import (  # Assuming backtests could also be artifacts
    AuditVerdict,
    BacktestResult,
)
...
def generate_reports(report_type: str, output_path: str = "reports"):
    ...
    if report_type == "audit":
        ...
        for audit_file in audit_files:
            artifact_json, stored_checksum = artifact_manager.download_artifact(
                audit_file.name
            )
            if not artifact_json or not stored_checksum:
                logging.warning(
                    f"Could not read or find checksum for {audit_file.name}, skipping."
                )
                continue

            # Verify checksum before including in report
            if (
                crypto_utils.sha256_hash(artifact_json.encode("utf-8"))
                == stored_checksum
            ):
                ...
                logging.error(
                    f"Checksum mismatch for {audit_file.name}, flagging as corrupt."
                )

    elif report_type == "incidents":
        logging.info("Generating incident report from database.")
        incidents = state_manager.execute_query(
            "SELECT * FROM incidents ORDER BY timestamp DESC", fetch="all"
        )
        report_filename = os.path.join(
            output_path, f"incident_report_{datetime.now().strftime('%Y%m%d')}.json"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate various reports for the strategy optimizer."
    )
    parser.add_argument(
        "--type",
        type=str,
        required=True,
        choices=["audit", "incidents"],
